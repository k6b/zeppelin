...
sub UpdateDB {
	...
	$json = new JSON;
	&MakeRequest("GET", "$server_url/servers/detail");
	my $server_list = $content;
	$json_list = $json->decode($server_list);
	...
	$json = new JSON;
	&MakeRequest("GET", "$server_url/images/detail");
	my $backup_list = $content;
	$json_list = $json->decode($backup_list);
	...
	$json = new JSON;
	&MakeRequest("GET", "$server_url/images/");
	my $distro_list = $content;
	$json_list = $json->decode($distro_list);
